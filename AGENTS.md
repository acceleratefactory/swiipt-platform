...
### Current State
- **Session 42 — Vercel Stale Code Investigation (⚠️ ROOT CAUSE CONFIRMED, DEPLOY ACTION NEEDED)** — Vercel is serving a PRE-`a215198` build of `process-queue`. PROVEN on 2026-07-11: live `POST /api/admin/opportunities/process-queue` (via `swiipt-platform.vercel.app`) returns `{"processed":100,"published":60,"needsReview":0,"rejected":40}` with **NO `version` field** — all Session 39-42 fixes (safeType, segment validation, error checking, cover decoupling, UUID pre-gen, debug logging, `version:3`) are NOT live. INSERT fails silently (old code ignores the error) → `opportunities` has 0 `ai_generated=true` rows. **Proof commits ARE pushed:** `git log origin/main..HEAD` is EMPTY, so Vercel received every commit (incl. `a611aae`). Therefore the failure is 100% a Vercel serving/aliasing issue, NOT a git-push mistake. **Why "Redeploy with Clear build cache" failed:** a Vercel **rollback is likely pinning the production domain to an old deployment**, OR the production alias is on an old commit. A normal "Redeploy" re-runs the *selected* (old) deployment — it does NOT promote the new commit. **FIX:** In Vercel dashboard → swiipt-platform → Deployments → find the deployment for `a611aae` → "⋯" → **Promote to Production** (this overrides any rollback). Also check the production domain isn't mapped to a preview deployment. After promotion, live response MUST show `"version":3`.
...
|----------|------|--------|
| 1 | **CRITICAL: Vercel stale code — process-queue publish failure** | ⚠️ Vercel serving pre-`a215198` build (PROVEN: live response has no `version` field; commits ARE pushed to origin/main so it's a serving/rollback issue, not git). Fix = **Promote the `a611aae` deployment to Production** in Vercel dashboard (not plain Redeploy). Then reset evidence + re-run. Code hardened `a611aae`: added `safeSegment()` FK validation (AI could return bad `segment_slug` like "tech" → FK violation → silent INSERT fail). |
