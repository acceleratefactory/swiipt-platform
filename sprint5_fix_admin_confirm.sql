-- =============================================================
-- Fix: Admin confirm button does nothing silently
-- Run ALL 4 blocks in order in Supabase SQL Editor
-- =============================================================

-- -------------------------------------------------------------
-- BLOCK 1: Check current state (paste result back)
-- -------------------------------------------------------------
SELECT
  ...
  (SELECT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'increment_mobility_score'))::text AS mobility_score_function_exists,
  (SELECT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'deposits' AND cmd = 'UPDATE'))::text AS deposits_update_policy_exists;

-- -------------------------------------------------------------
-- BLOCK 2: Missing RLS UPDATE policy for deposits table
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- BLOCK 3: increment_mobility_score (dependency)
-- -------------------------------------------------------------
CREATE OR REPLACE FUNCTION increment_mobility_score(user_id_input UUID, points INTEGER)
RETURNS VOID AS $$
BEGIN
  UPDATE users SET mobility_score = COALESCE(mobility_score, 0) + points WHERE id = user_id_input;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- -------------------------------------------------------------
-- BLOCK 4: confirm_deposit RPC (latest version with visa handling)
-- -------------------------------------------------------------
