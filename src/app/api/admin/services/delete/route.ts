...
export async function POST(request: NextRequest) {
  ...
  const { data: pkg } = await db.from("service_packages").select("name, is_active").eq("id", id).single();
  ...
  if (orderCount && orderCount > 0) {
    return NextResponse.json({
      error: `Cannot delete — ${orderCount} order(s) reference this package. Deactivate it instead.`,
    }, { status: 409 });
  }
  ...
  await db.from("admin_audit_log").insert({
    admin_id: user.id,
    action_type: "service_package_deleted",
    target_record_id: id,
    target_table: "service_packages",
    previous_value: JSON.stringify({ name: pkg.name, is_active: pkg.is_active }),
    new_value: null,
    note: `Deleted service package "${pkg.name}"`,
  });
