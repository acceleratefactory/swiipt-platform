...
export default function DocumentReviewCard({ doc, actionLoading, onVerify, onViewDocument }: { doc: any; actionLoading: string | null; onVerify: (documentRequestId: string, action: "approve" | "reject", reason?: string) => void; onViewDocument: (filePath: string) => void }) {
    ...
    <div style={{ background: 'white', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '1.25rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexShrink: 0 }}>
          <button
            onClick={() => onViewDocument(doc.file_url)}
      ...
      {showRejectForm && (
        <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
          ...
          />
          <button
            onClick={() => { onVerify(doc.id, "reject", rejectionReason); setShowRejectForm(false); setRejectionReason(""); }}
