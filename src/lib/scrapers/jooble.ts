...
export async function joobleScraper(
  pageUrl: string,
  sourceName: string,
  maxItems: number = 20
): Promise<EvidenceRecord[]> {
  const records: EvidenceRecord[] = [];
  const url = pageUrl || `${BASE_URL}/jobs-overseas-construction`;
  ...
  while ((m = linkRe.exec(html))) {
    ...
    const locMatch = snippet.match(/(?:location|city|place)[:\s]*<[^>]+>([^<]+)</i);
    const location = locMatch ? stripHtml(locMatch[1]).trim() : "";
    listings.push({ title: linkText.slice(0, 300), url: abs, organisation: org || "Jooble", location: location || "Overseas" });
  }
  ...
  if (records.length === 0) {
    ...
    if (extracted.length > 30) {
      const record = makeRecord(
        "Jooble — Overseas Construction & Trades Jobs",
        extracted.slice(0, 2000),
        url,
        sourceName,
        url,
        { organisation: "Jooble", location: "Overseas" }
      );
      if (record) records.push(record);
    }
  }
