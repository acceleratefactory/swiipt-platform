...
const SEARCH_URL = "https://www.glassdoor.com/Job/remote-jobs-SRCH_KO0,6.htm";
...
export async function glassdoorRemoteScraper(
  pageUrl: string,
  sourceName: string,
  maxItems: number = 20
  ...
  while ((lm = linkRe.exec(html))) {
    ...
    const organisation = orgMatch ? orgMatch[2].trim().slice(0, 200) : "Glassdoor Employer";
  ...
  if (records.length === 0) {
    ...
    if (extracted.length > 30) {
      const record = makeRecord(
        ...
        sourceName,
        pageUrl || SEARCH_URL,
        { organisation: "Glassdoor", location: "Remote" }
      );
      if (record) records.push(record);
    }
  }
