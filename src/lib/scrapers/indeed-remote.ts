...
const SEARCH_URL = "https://www.indeed.com/q-remote-jobs.html";
...
export async function indeedRemoteScraper(
  pageUrl: string,
  sourceName: string,
  maxItems: number = 20
  ...
  const html = await fetchWithRetry(pageUrl || SEARCH_URL, 25000, 3);
  if (!html) return [];
  ...
  while ((lm = linkRe.exec(html))) {
    ...
    if (!/viewjob|\/rc\//i.test(href)) continue;
    if (linkText.length < 4) continue;
    const abs = absolutizeUrl(href, BASE_URL).split("#")[0];
    const orgMatch = linkText.match(/^(.*?)\s*(?:at|,)\s*(.+)$/i);
    const organisation = orgMatch ? orgMatch[2].trim().slice(0, 200) : "Indeed Employer";
    listings.push({ title: linkText.slice(0, 300), url: abs, organisation });
  }

  for (const item of listings.slice(0, maxItems)) {
    const record = makeRecord(item.title, "", item.url, sourceName, pageUrl || SEARCH_URL, {
  ...
  if (records.length === 0) {
    ...
    if (extracted.length > 30) {
      const record = makeRecord(
        "Indeed — Global Remote Jobs",
        extracted.slice(0, 2000),
        pageUrl || SEARCH_URL,
        sourceName,
        pageUrl || SEARCH_URL,
        { organisation: "Indeed", location: "Remote" }
      );
      if (record) records.push(record);
    }
  }
