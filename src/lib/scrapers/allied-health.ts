...
const BASE_URL = "https://www.alliedhealthcareers.com";
...
export async function alliedHealthScraper(
  pageUrl: string,
  sourceName: string,
  maxItems: number = 20
  ...
  while ((m = linkRe.exec(html))) {
    ...
    if (!/(?:job|career|radiology|health|staffing|place|opportunit)/i.test(linkText + href)) continue;
    if (linkText.length < 4) continue;
    const abs = absolutizeUrl(href, BASE_URL).split("#")[0];
    listings.push({ title: linkText.slice(0, 300), url: abs, organisation: "Allied Health Careers" });
  }
  ...
  if (records.length === 0) {
    ...
    if (extracted.length > 30) {
      const record = makeRecord(
        "Allied Health Careers — Healthcare Staffing",
        extracted.slice(0, 2000),
        pageUrl || BASE_URL,
        sourceName,
        pageUrl || BASE_URL,
        { organisation: "Allied Health Careers", location: "United States" }
      );
      if (record) records.push(record);
    }
  }
